...
var interactiveSelectors =
  'a,button,input,select,textarea,.mejs-overlay-button,map,[onclick],[data-useswipe="1"],[data-tapstart="1"],.panzoom,#viewer-options-wrap';
var sliderSettings = {},
  nav = {},
  in5 = {
    layouts: [
...
function go(e, objArr, triggerEvent) {
  if (triggerEvent === "pageload") {
    var now = new Date().getTime(),
      goContent = JSON.stringify(objArr);
    if (
      in5.lastGoStart &&
      now - in5.lastGoStart < 200 &&
      in5.lastGoContent == goContent
    ) {
      return;
    }
...
function playAnim(e, item, objArr) {
  new AnimationItem(objArr, { pointer: -1, next: function () {} }).go();
}
function AnimationSequence(e, objArr, triggerEv) {
  (this.triggerEvent = triggerEv),
  ...
  this.update = function () {
    this.endCount++;
    if (this.endCount === this.length) {
      $(document).trigger("animationSequenceEnd");
    }
  };
  this.next = function () {
...
AnimationSequence.prototype.toString = function () {
  return this.name;
};

function AnimationGroup(parentSeq, startIndex) {
  (this.children = []),
    (this.sequence = parentSeq),
    (this.startIndex = startIndex),
    (this.loopCount = 0),
    (this.pointer = 0);
  this.push = function (animItem) {
    this.children.push(animItem);
    animItem.group = this;
    animItem.groupIndex = this.children.length - 1;
    if (!animItem.link && animItem.groupLoops > 1) {
      this.loops = animItem.groupLoops;
      this.lastItem = animItem;
      animItem.$element.one(
        animItem.loops !== 1 ? animationItEvents : animationEndEvents,
        function (e) {
          var aItem = $(this).data("animationItem"),
            aGroup = aItem.group;
          if (!aItem.groupMode) {
            aItem.next();
          }
...
AnimationGroup.prototype.toString = function () {
  return this.name;
};
...
function AnimationItem(obj, seq) {
  (this.data = obj),
    (this.dataID = obj.id),
    (this.$element = $("[data-id=" + this.dataID + "]").not(
      "video,audio,object"
    )),
    (this.element = this.$element[0]),
    (this.action = obj.act),
    (this.sequence = seq),
    (this.animCSS = this.$element.attr("data-ani")),
    (this.loops = obj.n),
    (this.groupLoops = obj.sn),
    (this.delay = obj.del),
    (this.hideStart = this.$element.attr("data-hidestart") === "1"),
    (this.hideEnd = this.$element.attr("data-hideend") === "1"),
    (this.unroll = obj.unroll),
    (this.link = obj.link),
    (this.index = seq.pointer);
  (this._prep = function () {
    switch (this.action) {
      case "reverse":
        this.reverse();
      ...
      case "play":
        ...
        return !1;
      case "stop":
        this.$element.css(pre + "animation", "none");
        return !1;
      case "pause":
        this.$element.css(pre + "animation-play-state", "paused");
        break;
      case "resume":
        this.$element.css(pre + "animation-play-state", "running");
        break;
      case "stopall":
        $(".activePage")
          .find("[data-ani]")
          .css(pre + "animation", "none");
        break;
    }
  }),
    (this._forceRedraw = function () {
      this.element.offsetWidth = this.element.offsetWidth;
    }),
    (this.reset = function () {
      this.$element.css(pre + "animation", "");
      this.active = !1;
      return this;
    }),
    (this.stop = function () {
    ...
    (this.play = function () {
      ...
      var ani = this.animCSS,
        delay = this.delay ? this.delay + "s" : "0s",
        dir = this._reverse ? "reverse" : "normal";
      if (this.loops && this.loops != 1)
        ani = ani.replace(/(?:\d+$)/, this.loops);
      if (!this._unrolling) {
        if (this.unroll) {
          this.setRollOff();
        }
        if (this.sequence.length) {
          var animEvent = this.playSync
            ? animationStartEvents
            : this.loops !== 1
            ? animationItEvents
            : animationEndEvents;
          this.$element
            .data("animationItem", this)
            .one(animEvent, function (e) {
      ...
      this.$element
        ...
        .removeClass("hidden");
      if (
        this.$element.parents(".pageItem").hasClass("hidden") ||
        !this.$element.parents(".activePage").length
      ) {
        this.next();
      }
      return this;
    ...
    (this.setRollOff = function () {
      $(this.unroll)
        .data("unrollItem", this)
        .one("mouseleave", function (e) {
          $(this).data("unrollItem").doRollOff();
        });
    }),
    (this.next = function () {
      var parent = this.groupMode ? this.group : this.sequence;
      parent.update();
      this.active = !1;
      parent.next();
    }),
...
AnimationItem.prototype.toString = function () {
  return this.name;
};
...
function toggleAudio(btn) {
  ...
  try {
    var player = elem.player || elem,
      media = player.media || elem;
    if (media.paused) player.play();
    else player.pause();
  } catch (e) {}
}

function resumeMedia(dataID) {
  playMedia(dataID, -1);
}
...
function stopAllMedia(targ, opts) {
  ...
  $(targ)
    ...
    .each(function () {
      ...
      if (!opts.force && media.getAttribute("data-nostop") == "1") {
        return !1;
      }
      try {
        player.pause();
...
function autoPlayMedia(i, elem) {
  if (
    ...
    $(elem).parents(".state").not(".active").length
  )
    return; /*skip hidden video in other layouts or MSO states*/
...
function onNewPage(e, data) {
  ...
  var activePages = [],
    $pages = $(".page").removeClass("activePage"),
    $active;
  if (data.view) {
  ...
  $active.each(function (index, el) {
    var $el = $(el).addClass("activePage").show(),
      aniLoad = $el.find(".page-scale-wrap:visible").attr("data-ani-load");
    $el
      .find("audio,video")
      .filter("[data-autoplay]")
      .each(function (i, elem) {
        autoPlayMedia(i, elem);
      });
  ...
  if (!multifile && resetMSOs) {
    $(".page-scale-wrap .mso").each(function (ind, elem) {
      toFirstState(elem);
    });
  } else {
...
function onNewState(e) {
  ...
  targState
    .find("audio,video")
    .filter('[data-autoplay="1"]')
    .each(function (i, elem) {
      autoPlayMedia(i, elem);
    });
  targState.find('[data-autostart="1"]').each(function (i, el) {
    toFirstState(el);
    startSlideShowDelayed(el);
  });
  $otherStates = targState.siblings(".state");
  $otherStates.find("[data-hidestart]").addClass("hidden");
  if (targState.parents(".activePage").length) {
  ...
  mso.each(function (index, elem) {
    if (elem.hasOwnProperty("loopcount")) {
      ...
      if (elem.loopmax != -1 && elem.loopcount >= elem.loopmax) {
        stopSlideShow(elem);
        $(document).trigger("slideShowComplete", { mso: mso });
        return !0;
      }
    }
    if (elem.crossfade > 0) {
      ...
      var last = el
        ...
        .addClass("transition")
        .show()
        .fadeOut(elem.crossfade, function () {
          $(this).removeClass("transition");
        });
      var ondeck = rev
        ? el.children(".state").last()
        : el.children(".state").first();
      ondeck
        .addClass("active")
        .hide()
        .fadeIn(elem.crossfade, function (e) {
          last.hide();
        })
        .trigger("newState");
    } else if (rev) {
...
function nextState(dataID, loop) {
  ...
  } else if (loop && checkSlideLoop(mso)) {
    return;
  }
}
...
function prevState(dataID, loop) {
  ...
  } else if (loop && checkSlideLoop(mso, !0)) {
    return;
  }
}
...
function toState(dataID, stateIndex, restoreOnRollOut, restoreTarg) {
  if (restoreOnRollOut) {
    var current = $("[data-id=" + dataID + "]")
      ...
      .first()
      .index();
    $(restoreTarg).mouseleave(function () {
      toState(dataID, current);
    });
  }
  $("[data-id=" + dataID + "]").each(function (index, elem) {
    var $el = $(elem);
    if ($el.attr("class").indexOf("flipcard-") > -1) {
      $el[stateIndex > 0 ? "addClass" : "removeClass"]("flipped")
        .trigger("newState")
        .children()
        .attr("display", "");
      return !1;
...
function toFirstState(el) {
  var f = el.reverse ? $(el).children(".state").length - 1 : 0;
  setTimeout(function () {
    toState($(el).attr("data-id"), f);
  }, 1);
}
...
function startSlideShowDelayed(el) {
  ...
  setTimeout(function () {
    startSlideShow(el);
  }, parseFloat(mso.attr("data-autostartdelay")) * 1000 +
    (mso.is(":visible") ? el.duration * 1000 : 0));
}
...
function startSlideShow(el) {
  ...
  el.playint = setInterval(function () {
        ...
        switch (at) {
          case "onclick":
            ...
            clickArr.push(function (event) {
              eval($el.attr("data-onclick")); /*name must be 'event'*/
            });
            break;
          case "data-ani-click":
            clickArr.push(function (e) {
              ...
              if ($el.parent().hasClass("activePage")) eval(aval);
            });
            break;
          ...
          case "data-click-hide":
            ...
            break;
          case "data-click-next":
            clickArr.push(function (e) {
              args = $el.attr("data-loop") == "1";
              $.each(aval.split(","), function (i, val) {
                nextState(val, args);
              });
            });
            break;
          case "data-click-prev":
            clickArr.push(function (e) {
              args = $el.attr("data-loop") == "1";
              $.each(aval.split(","), function (i, val) {
                prevState(val, args);
            ...
            break;
          case "data-click-state":
            clickArr.push(function (e) {
              $.each(aval.split(","), function (i, val) {
                args = val.split(":");
                toState(args[0], args[1]);
              });
            });
            break;
          case "data-click-play":
            clickArr.push(function (e) {
              $.each(aval.split(","), function (i, val) {
                args = val.split(":");
                playMedia(args[0], args[1]);
              });
            });
            break;
          case "data-click-resume":
            clickArr.push(function (e) {
              $.each(aval.split(","), function (i, val) {
                args = val.split(":");
                resumeMedia(args[0]);
          ...
          case "data-click-stop":
            ...
            break;
          case "data-click-stopall":
            clickArr.push(function (e) {
              stopAllMedia(undefined, { src: "btn" });
            });
            break;
        }
      });
...
function initAnimationAttribs() {
  ...
  $ani
    ...
    .each(function (index, el) {
      var $el = $(el),
        hs = $el.attr("data-hidestart") == "1",
        he = $el.attr("data-hideend") == "1";
      if (!multifile && (hs || he)) {
        $(document).on("newPage", function (e, data) {
          var onpage = $.contains($(".page").eq(data.index)[0], el),
            style = $el.attr("style") || "";
...
function initMSOs() {
  ...
  $msos.filter(".flipcard-up,.flipcard-side").on(clickEv, function (e) {
    if (!$(e.target).parents("a").length) {
      $(this).toggleClass("flipped");
      return !1;
    }
  });
  $msos.find(".state").on("newState", function (e) {
    onNewState(e);
  });
  $msos.filter(".slideshow").each(function (index, el) {
    var mso = $(el),
      msoID = mso.attr("data-id"),
      loopSwipe = mso.attr("data-loopswipe") == "1";
    ...
    if (msoSwipe) {
      if (useSmoothSwipeOnImageSequences && mso.hasClass("seq")) {
        ...
        mso
          ...
          .on(pointerEnabled ? "pointerdown" : "vmousedown", function (e) {
            in5.msoTarget =
              (e.target == el || $(e.target).parents(el).length) &&
              $(e.target).parents(".activePage")
                .length; /*track swipe trig by mso*/
          })
          .swipe({
            allowPageScroll: "vertical",
            fingers: 1,
            maxTimeThreshold: 9999,
            ...
            swipeStatus: function (
              ...
              switch (phase) {
                ...
                case "start":
                  lastPos = 0;
                  triggerDist = (mso.width() / mso.find(".state").length) * 0.5;
                  break;
                case "end":
                  animateImageSeq(
                    currentDir,
                    ...
                    msoID,
                    loopSwipe,
                    calculateVelocity(evt, dist, dur),
                    Date.now()
                  );
                  break;
              }
...
function initLightbox() {
  $(".lightbox")
    .filter(":not(svg *)")
    .filter(":not([href*=lightbox\\=0])")
    .filter(isBaker ? ":not([href*=referrer\\=Baker])" : "*")
    .colorbox({ iframe: !0, width: "85%", height: "85%" });
  ...
  $("[aria-details]").each(function (index, el) {
    var jel = $(el),
      det = jel.attr("aria-details"),
      jid = jel.attr("id") || "",
      ext = det.slice(-5) === ".html";
    jel.on(clickEv, function () {
      if (ext) {
        var colorboxProps;
        if (isWebView) {
          var maxW = deviceDimensions.width * 0.85,
            maxH = deviceDimensions.height * 0.85,
            top = (deviceDimensions.height - maxH) * 0.5,
            left = (deviceDimensions.width - maxW) * 0.5;
          colorboxProps = {
            iframe: !1,
            width: "85%",
            maxWidth: maxW,
          ...
          colorboxProps = {
            iframe: isLocal && pre.length /*chrome loc fallback*/,
            width: "85%",
            maxWidth: "600px",
            height: "85%",
            href: det,
  ...
  if (multifile) {
    ...
    nav = {
      ...
      to: function (n, coords) {
        ...
        if (coords && coords.length)
          targPage += "#refx=" + coords[0] + "&refy=" + coords[1];
    ...
    nav = {
      ...
      reposition: function (view) {
        ...
        if (dispNum === 1) {
          $sl.css(prefix.css + "transform", "translateX(0px)");
          return;
        }
        if (!view[0] || !view[1]) {
    ...
    nav = {
      ...
      to: function (n, c) {
        ...
        if (c) {
          scrollTarg = {
            left: scrollTarg.left + c[0],
            top: scrollTarg.top + c[1],
          };
        }
...
function initPageSwipe() {
  ...
  container.swipe({
    ...
    excludedElements:
      ...
      ',.mejs-overlay-button,map,[onclick],[data-useswipe="1"],[data-tapstart="1"],.panzoom,.scroll-horiz,#viewer-options-wrap',
...
function addNavProps() {
  var pages_count = $("li").length;
  if (nav.numPages === undefined) nav.numPages = pages_count;
  ...
  nav.update = function (n) {
    if (arrowNav && nav.numPages > 1) {
      ...
      if (n ? n < 2 : flip ? view[0] < 2 || view[1] < 2 : nav.current < 2)
        $("nav #backBtn").hide();
      if (
...
function scaleLayout(getOnly, sf) {
  var targ = (multifile || $(".activePage").is(":empty")
      ? $(".page")
      : $(".activePage")
    )
      .find(".page-scale-wrap")
...
function scaleFlipLayout(getOnly, sf) {
  ...
  isSafari
    ? $(".page-scale-wrap").css("zoom", scaleFactor)
    : $(".page-scale-wrap")
        .css(
          prefix.css + "transform",
...
function isFullscreen() {
  return window.fullScreen || window.fsmode;
}
...
function updateCurrentLayout() {
  var $psw = $(multifile ? ".page" : ".page.activePage").find(
    ".page-scale-wrap:visible"
  );
...
function initMedia(hasSlider) {
  ...
  var playerMode =
    uAgent.indexOf("firefox") > -1 &&
    isLocal &&
    !document
      .createElement("video")
      .canPlayType('video/mp4; codecs="avc1.42E01E"').length
      ? "shim"
      : "auto";
  if (
    (isIPad || isIPhone) &&
    $("audio,video").filter("[data-autoplay]").length
  ) {
  ...
  if (hasSlider && (isIPad || isIPhone))
    $("video,audio").mediaelementplayer({ success: onMediaLoadSuccess });
  else {
...
function onMediaLoadSuccess(me, domObj) {
  mediaLoaded++;
  var $domObj = $(domObj);
  $(document).trigger("mediaElementLoaded", {
    mediaElement: me,
    element: domObj,
  });
  if ($domObj.hasClass("mejs-fsonly")) {
    me.addEventListener("play", function () {
      try {
        domObj.player.enterFullScreen();
        me.enterFullScreen();
      } catch (e) {}
    });
  }
  if ($domObj.attr("data-stoplast") == "1") {
    if (domObj.hasOwnProperty("player")) domObj.player.options.autoRewind = !1;
  }
  me.addEventListener("play", function () {
    $(document).trigger("mediaPlayback", { me: me, domObj: domObj });
  });
  me.addEventListener("ended", function (e) {
    $domObj
      ...
      .show()
      .find("img")
      .show(); /*reshow poster when done*/
  });
  if (me.pluginType == "flash" && $domObj.attr("loop") == "loop") {
    me.addEventListener("ended", function () {
      domObj.player.play();
    });
  }
  if (mediaLoaded === $("audio,video").length) allMediaLoaded();
  var cc = $domObj.attr("data-cc-on"),
    cin = $(me)
      .parents(".mejs-container")
      .find('.mejs-captions-selector input[value="' + cc + '"]')
      .prop("checked", !0);
  if (cc) {
    me.player.setTrack(cc);
  }
  $domObj.filter("[data-end-action]").on("ended", function (e) {
    var action = $(this).attr("data-end-action"),
    ...
    switch (actionArr[0]) {
      case "next":
        ...
        break;
      case "mso":
        toState(actionArr[1], actionArr[2]);
        break;
    }
  });
  if ($domObj.attr("data-overlap") == "1" && domObj.player)
    domObj.player.options.pauseOtherPlayers = !1;
}
